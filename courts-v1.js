...
function createMarkers() {
    ...
    const marker = L.marker(t.coord, { icon: hoopIcon })
      .bindPopup(`<div class='court-popup'>
        ...
        <div class='court-popup-content'>
          <p class='court-popup-description'>${t.description}</p>
        </div>
      </div>`);
    courtMarkers.push(marker);
    if (document.getElementById('toggle-courts').classList.contains('active')) {
      courtClusterGroup.addLayer(marker);
    }
  });

  multiSportCourts.forEach(t => {
    ...
    multisportCourtMarkers.push(marker);
    if (document.getElementById('toggle-multisport-courts').classList.contains('active')) {
      multisportCourtClusterGroup.addLayer(marker);
    }
  });
  ...
  clubs.forEach(c => {
    const marker = L.marker(c.coord, {
      ...
      .bindPopup(`<div class='court-popup'>
        <div class='court-popup-header'>
          <img src='${c.iconFileName}' alt='${c.nom}' class='court-popup-icon'>
          <h3 class='court-popup-title'>${c.nom}</h3>
        </div>
        <div class='court-popup-content'>
          <p class='court-popup-description'>${c.description}</p>
        </div>
      </div>`);
